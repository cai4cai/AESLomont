...
/* USAGE:
        ...
        3. To encrypt, call method StartEncryption with the key, and then
                call method Encrypt with enough space to store the proper size
   blocks.
        4. To decrypt, call method StartDecryption with the key, and then
                call method Decrypt with enough space to store the proper size
   blocks.

   Alternatively, you can call EncryptBlock and DecryptBlock block to process
   blocksize (default to 16 bytes) bytes at a time. It is recommended to use the
   Encrypt function for multiple block encryption since it uses chaining modes
   that make the overall stream much more secure than the default block based
   encryption, which by default would be mode ECB.

   EXAMPLE: want to encrypt 37 bytes of data with 192 bit key, which will use 3
   16 byte blocks AES aes; aes.SetParameters(192); aes.StartEncryption(key);
   aes.Encrypt(data,output,3); // note data and output must be at least 48
   bytes!
  */
...
class AES {
 public:
  ...
  // independently. NOTE: the AES standard only uses a blocksize of 128, so we
  // default to that
  ...
  // Call this to encrypt any length data. Note the size is in BLOCKS, so you
  // must have enough space in datain and dataout to accomodate this. Pad your
  ...
  // Call this to decrypt any length data. Note the size is in BLOCKS, so you
  // must have enough space in datain and dataout to accomodate this. Pad your
  // data before calling, preferably using the padding methods listed below. You
...
same block is encrypted twice with the same key, the resulting ciphertext blocks
are the same. In CBC mode, a ciphertext block is obtained by first XORing the
plaintext block with the previous ciphertext block, and encrypting the resulting
value. In CFB mode, a ciphertext block is obtained by encrypting the previous
ciphertext block and XORing the resulting value with the plaintext. The
operation mode is specified in the iMode parameter with ECB being the default
value.
